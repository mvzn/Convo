...
DIST_XML="$WORK/distribution.xml"
{
    ...
    for ref in "${PKG_REFS[@]}"; do
        echo "    <choice id=\"$ref\" visible=\"false\"><pkg-ref id=\"$ref\"/></choice>"
...
PB_ARGS=(--distribution "$DIST_XML" --package-path "$WORK")
