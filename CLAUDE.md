...
- Use `juce::SmoothedValue` for all audio-rate gain/param changes (dry, wet, IR Gain, output, tone, width, duck, bypass, wet comp, input HP/LP cutoffs, bass-mono cutoff, clip-guard blend).
- The pre-IR input filter (first-order HP/LP, 6 dB/oct) sits on the wet source before convolution; the dry tap is never filtered. **Filter IR** instead bakes the HP/LP into the kernel in `bake()` (after auto-level, so the result is identical by LTI). Mid/Side is a bake param (`bake()` re-encodes the kernel to M/S), with an optional **Bass Mono** high-pass on the side; the audio thread M/S-encodes/decodes and runs the input filter only when the live kernel matches (published via the `msActive` / `filterInput` atomics).
- Routing toggles (Mid/Side, Filter IR) re-bake and arm the load fade; filters reset on re-engage and the Clip Guard is gain-blended (`clipGuardSm`). Keep all toggles click-free.
